...
├── io/
│   ├── gtfs.py          # feed loading, validation, peak frequency table  [implemented]
...
| Concern | Choice | Why |
| --- | --- | --- |
| GTFS parsing | hand-rolled `pandas` reader (`io/gtfs.py`) | see deviation note below |
...
### Deviation: GTFS parser (Phase 1)

The original design named `partridge` (or `gtfs-kit`) for GTFS parsing. The implemented
reader (`io/gtfs.py`) instead reads the feeds directly with `pandas`. Reasons:

- **Per-feed id namespacing.** Indian cities are served by several operators that each
  publish their own zip with overlapping ids (`route_id = "1"` in both bus and metro
  feeds). The reader namespaces every id with its feed key (`bmtc:1`) on load so feeds
  merge without collisions — easier to do with direct control over the dataframes.
- **Readability & explicitness.** A transparent read → validate → resolve-service-date →
  filter-window → groupby pipeline is easier to audit against `methodology.md` than a
  third-party feed object.
- **Efficiency we control.** Only the needed columns are read (`usecols`), the large
  `stop_times` table is filtered to active trips before times are parsed, and counting
  is fully vectorised — no per-stop Python loops.

`partridge` remains declared in `pyproject.toml` for now; it can be reinstated behind the
same `load_feed`/`load_feeds` API if a need arises (e.g. very large feeds). The public
interface does not depend on the parser choice.
