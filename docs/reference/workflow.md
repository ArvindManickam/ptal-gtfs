...
Each line shows the elapsed time since the run began. `save()` **continues the same clock**,
so any gap between compute and save (e.g. an OS swap stall) is visible rather than hidden by
a reset:

```python
analysis.compute(verbose=True)
# [ptal     0.0s] loading GTFS feeds + peak frequencies ...
# [ptal    31.3s]   10,821 stops, 2,439 routes
# [ptal    31.3s] downloading OSM walk network from Overpass (usually the slow part) ...
# [ptal    99.0s]   walk graph 349,106 nodes / 976,548 edges
# [ptal   521.9s] done in 521.9s - 250,171 cells scored
```
